...
class OrderProductService:
    ...
    def add_order_product(self, order_id: int, payload: OrderProductCreateDTO) -> OrderProductRead:
        self._ensure_order_exists(order_id)
        self._ensure_product_exists(payload.product_id)
        order_product = self._repository.create(order_id, payload)
        return OrderProductRead.model_validate(order_product)

    def update_order_product(
        self, order_id: int, order_product_id: int, payload: OrderProductUpdateDTO
    ) -> OrderProductRead:
        self._ensure_order_exists(order_id)
        self._ensure_product_exists(payload.product_id)
        order_product = self._repository.update(order_id, order_product_id, payload)
        if order_product is None:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Order product not found")
        return OrderProductRead.model_validate(order_product)
